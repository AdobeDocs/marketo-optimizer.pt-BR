---
user-guide-title: Documentação do Marketo Otimizer
user-guide-description: Saiba mais sobre o Adobe Marketo Otimizer e como você pode usá-lo para executar o marketing coordenado e o envolvimento de vendas com conteúdo personalizado alimentado por IA para clientes potenciais e contas B2B.
hide: true
nudge: red
source-git-commit: d1268dd4fadec58b5adedeaa295ca0624c2c2dcd
workflow-type: tm+mt
source-wordcount: '221'
ht-degree: 46%

---


# Guia do usuário do Marketo Otimizer {#user}

+ [Documentação do Adobe Marketo Otimizer](guide-overview.md)
+ [Arquitetura de dados](data-architecture.md)
+ Introdução {#start}
  + [Configurar lista de verificação](./start/setup-check-list.md)
  + [Gerenciamento de usuários](./start/user-management.md)
  + [Capacidade de entrega de email](./start/email-deliverability.md)
+ [Página inicial](./home-insights/home-page.md)
+ Recursos de colega de trabalho {#coworker}
  + [Interface de chat](./agents/chat-interface.md)
  + [Habilidades](./agents/skills.md)
  + [Criar programa a partir do resumo](./agents/program-from-brief.md)
  + [Criar públicos-alvo para programas](./agents/audience-creation.md)
  + [Personalizar conteúdo de email por persona](./agents/personalize-content.md)
  + [Criar modelos de pontuação personalizados](./agents/lead-scoring-model.md)
  + [Criar e promover webinars](./agents/webinar-creation.md)
+ Decisão sobre IA {#ai-decisioning}
  + [Visão geral](./agents/ai-decisioning.md)
  + [Integridade da decisão sobre IA](./agents/ai-decisioning-health.md)
+ Gerenciamento de marketing {#marketing-management}
  + [Visão geral](./marketing/marketing-management.md)
  + Programas {#programs}
    + [Visão geral](./marketing/programs.md)
    + [Tokens personalizados para personalização](./marketing/personalization-my-tokens.md)
  + [Jornadas de pessoas](./marketing/person-journeys.md)
  + Jornada tela e nós {#journey-nodes}
    + [Visão geral dos nós](./marketing/person-journey-nodes.md)
    + [Público-alvo de pessoa](./marketing/person-audience-node.md)
    + [Realizar uma ação](./marketing/action-nodes.md)
    + [Acompanhar um evento](./marketing/listen-for-event-nodes.md)
    + [Aguardar](./marketing/wait-nodes.md)
    + [Dividir e mesclar caminhos](./marketing/split-merge-paths-nodes.md)
    + [Próximo caminho recomendado](./marketing/next-best-path.md)
  + Canal de email {#email-channel}
    + [Visão geral](./marketing/email-channel.md)
    + [Otimização do tempo de envio de email](./marketing/email-send-time-optimization.md)
    + [Criação de conteúdo](./content/email-authoring.md)
    + [Design de modo escuro](./content/email-dark-mode.md)
  + [Canal do WhatsApp](./content/whatsapp-authoring.md)
  + Canal do webinário {#webinar-channel}
    + [Visão geral](./marketing/webinars-overview.md)
    + [Criar um webinário](./marketing/create-webinar.md)
    + [Jornadas do webinário](./marketing/webinar-journeys.md)
+ [Jornada controle de tráfego](./marketing/journey-traffic-control.md)
+ Públicos-alvo {#audiences}
  + [Listas de pessoas](./audiences/people-lists.md)
  + [Dados da pessoa](./audiences/person-details.md)
  + [Públicos-alvo baseados em eventos](./audiences/event-based-audiences.md)
  + [Pontuações de engajamento da pessoa](./audiences/engagement-scores.md)
  + [Personas derivadas](./audiences/personas.md)
  + [Destinos](./audiences/destinations.md)
+ Conteúdo {#content}
  + Ativos {#assets}
    + [Visão geral](./content/digital-asset-management.md)
    + [Metadados do C2PA](./content/c2pa-metadata.md)
  + Modelos {#templates}
    + [Visão geral](./content/templates.md)
    + [Criar modelos](./content/templates-create.md)
    + [Governança de conteúdo](./content/template-content-governance.md)
  + Fragmentos {#visual-fragments}
    + [Visão geral](./content/fragments.md)
    + [Criação de fragmentos](./content/fragment-authoring.md)
  + Páginas de destino {#landing-pages}
    + [Visão geral](./content/landing-pages.md)
    + [Criar e publicar páginas de destino](./content/landing-pages-create-publish.md)
    + [Design da página de destino](./content/landing-page-design.md)
  + Formulários {#forms}
    + [Visão geral](./content/forms.md)
    + [Design de formulário](./content/form-design.md)
  + [Personalização](./content/personalization-expressions.md)
  + [Conteúdo condicional](./content/conditional-content.md)
  + Ferramentas de design de conteúdo {#content-design}
    + [Componentes da estrutura](./content/structure-components.md)
    + [Componentes do conteúdo](./content/content-components.md)
    + [CSS personalizado](./content/design-custom-css.md)
+ [Gerenciamento de privacidade](./admin/privacy-management.md)
+ Administração {#admin}
  + Canais {#channels}
    + [Configuração do canal de email](./admin/email-channel-configuration.md)
    + [Configuração de canal do WhatsApp](./admin/configuration-channels-whatsapp.md)
    + [Configuração da landing page](./admin/configuration-presets-landing-pages.md)
    + [Configuração da predefinição do formulário](./admin/configuration-presets-forms.md)
  + [Regras de negócios](./admin/business-rules.md)
  + [Tipos de programas](./admin/program-types.md)
+ [Qualificador de Vendas](./sales-qualifier/sales-qualifier.md)

<!--
   + [Persona mapping](./admin/persona-mapping.md)
-->
